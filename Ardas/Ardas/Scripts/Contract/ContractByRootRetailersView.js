...
function GetRootRetailer() {
    ...
    var divRoot = document.getElementById('rootSelectRetailer');
    ...
    CallLoadServerMethod(methodName, null, function (result) {
        if (result.length != 0) {
            ...
            result.forEach(function (item, i) {
                ...
                optionOneRootRetailer.setAttribute('onclick', 'GetContractByRootRetailer()');
                select.appendChild(optionOneRootRetailer);
            });

            divRoot.appendChild(select);
            GetContractByRootRetailer();

        }
    });
}

function GetContractByRootRetailer()
{
    var loadImage = document.getElementById('load');
    loadImage.style.display = 'block';

    var rootContracts = document.getElementById('rootContracts');
    var methodName = "GetContractByRootRetailers";
    var request = {};
